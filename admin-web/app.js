...
async function enterAdmin(){
  try{
    ...
    if(!Array.isArray(rows)||rows.length===0){ $('noAccessView').classList.remove('hidden'); $('dashboard').classList.add('hidden'); return; }
    $('noAccessView').classList.add('hidden'); $('dashboard').classList.remove('hidden');
    await loadAll();
  }catch(e){ clearSession(); $('dashboard').classList.add('hidden'); $('noAccessView').classList.add('hidden'); $('loginView').classList.remove('hidden'); msg($('authMessage'),friendly(e),'error'); }
}
...
async function loadAll(){
  ...
  try{
    const [c,p,e]=await Promise.all([
      ...
      api('/rest/v1/subscription_events?select=id,event_type,old_status,new_status,old_plan,new_plan,notes,created_at,clinic:clinics(name)&order=created_at.desc&limit=80')
    ]);
    clinics=Array.isArray(c)?c:[]; pendingPayments=Array.isArray(p)?p:[]; events=Array.isArray(e)?e:[];
    renderStats(); renderClinics(); renderPayments(); renderEvents(); fillClinicSelect();
    msg($('globalMessage'),'تم التحديث','success');
  }catch(e){ msg($('globalMessage'),friendly(e),'error'); }
}

function renderStats(){
  const count=s=>clinics.filter(c=>c.subscription_status===s).length;
  const values=[['كل العيادات',clinics.length],['تجربة',count('trialing')],['نشط',count('active')],['موقوف/متأخر',count('suspended')+count('past_due')],['دفعات معلقة',pendingPayments.length]];
  $('stats').innerHTML=values.map(([l,v])=>`<div class="stat"><span>${esc(l)}</span><strong>${v}</strong></div>`).join('');
}
...
function renderClinics(){
  ...
  $('clinicsBody').innerHTML=rows.length?rows.map(c=>`<tr>
    ...
    <td><div class="actions">
      ...
      ${c.subscription_status!=='active'?`<button class="secondary" data-clinic="${esc(c.id)}" data-status="active">تنشيط</button>`:''}
...
function renderPayments(){
  $('paymentsBody').innerHTML=pendingPayments.length?pendingPayments.map(p=>`<tr>
    ...
    <td>${esc(p.requested_plan)} / ${p.requested_days} يوم</td><td>${esc(p.reference||'—')}</td>
...
function fillClinicSelect(){
  $('manualClinic').innerHTML=clinics.map(c=>`<option value="${esc(c.id)}">${esc(c.name)} — ${esc(statusLabel(c.subscription_status))}</option>`).join('');
}
