    ...
    public static void install(Context context) {
        try {
            ...
            try {
                ...
                db.execSQL("CREATE TRIGGER IF NOT EXISTS trg_sync_patients_insert AFTER INSERT ON patients BEGIN " +
                ...
                db.execSQL("CREATE TRIGGER IF NOT EXISTS trg_sync_patients_update AFTER UPDATE ON patients BEGIN " +
                        "INSERT OR IGNORE INTO sync_entity_keys(entity_type,local_id,sync_key) VALUES('patient',NEW.id,lower(hex(randomblob(16)))); " +
                        "INSERT OR REPLACE INTO sync_dirty(entity_type,local_id,changed_at) VALUES('patient',NEW.id,STRFTIME('%Y-%m-%d %H:%M:%f','now')); END");

                db.execSQL("CREATE TRIGGER IF NOT EXISTS trg_sync_visits_insert AFTER INSERT ON visits BEGIN " +
                ...
                db.execSQL("CREATE TRIGGER IF NOT EXISTS trg_sync_visits_update AFTER UPDATE ON visits BEGIN " +
                        "INSERT OR IGNORE INTO sync_entity_keys(entity_type,local_id,sync_key) VALUES('visit',NEW.id,lower(hex(randomblob(16)))); " +
                        "INSERT OR REPLACE INTO sync_dirty(entity_type,local_id,changed_at) VALUES('visit',NEW.id,STRFTIME('%Y-%m-%d %H:%M:%f','now')); END");

                // First install: existing rows must be included in the initial sync.
                ...
                db.execSQL("INSERT OR REPLACE INTO sync_dirty(entity_type,local_id,changed_at) SELECT 'patient',id,STRFTIME('%Y-%m-%d %H:%M:%f','now') FROM patients");
                db.execSQL("INSERT OR REPLACE INTO sync_dirty(entity_type,local_id,changed_at) SELECT 'visit',id,STRFTIME('%Y-%m-%d %H:%M:%f','now') FROM visits");
